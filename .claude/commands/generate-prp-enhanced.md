...
```bash
/generate-prp-enhanced <feature-file> [options]
```
...
- `/generate-prp-enhanced features/chat-ui.md --env typescript-env --include-dojo`
- `/generate-prp-enhanced features/user-api.md --env python-env --examples user-management`
- `/generate-prp-enhanced features/multi-agent.md --env multi --verbose`
- `/generate-prp-enhanced features/cli-tool.md --analyze-only`
...
source /Users/cedric/dev/github.com/polyglot-devenv/context-engineering/utils/template-engine.nu
source /Users/cedric/dev/github.com/polyglot-devenv/context-engineering/utils/dojo-integrator.nu
...
print $"🚀 Enhanced PRP Generation for: ($feature_file)"
...
match ($parsed_args.format) {
    "json" => {
        let json_output = {
            metadata: {
                generated_by: "generate-prp-enhanced",
    ...
    _ => {
        ...
        let markdown_header = $"<!-- Generated by generate-prp-enhanced -->
<!-- Timestamp: (date now | format date "%Y-%m-%d %H:%M:%S") -->
<!-- Source: ($feature_file) -->
<!-- Environment: ($target_env) -->
...
print ""
print "✅ Enhanced PRP generation completed!"
